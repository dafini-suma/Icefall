...
def compute_loss(
    ...
    with torch.set_grad_enabled(is_training):
        ...
        print("Output length:", len(output))  # Check the length of the returned values

        # If it returns more than 3 values, adjust accordingly
        if len(output) >= 3:
            simple_loss, pruned_loss, ctc_loss = output[:3]
        else:
            print("Model returned fewer than 3 values.")
...
def run(rank, world_size, args):
    ...
    dataset = AsrDataModule(args)

    train_cuts = dataset.train_cuts(args.train_cuts)
    ...
    def remove_short_and_long_utt(c: Cut):
        ...
        # for subsampling
        T = ((numpy.round(c.duration*100) - 7) // 2 + 1) // 2
    ...
    valid_cuts = dataset.valid_cuts(args.valid_cuts)
